...
* gen Absolute % Error = | Old - New | / OLD * 100 
...
foreach update of local list1 {
	...
	foreach location in XAB XBC XMB XNS XON XQC XSK XXX {
		...
		qui capture drop DayAPEMeSmA02S01`location'`update' 
        qui gen DayAPEMeSmA02S01`location'`update' = (abs(DayDeaMeSmA00S00`location' - DayDeaMeSmA02S01`location'`update') / (DayDeaMeSmA00S00`location')) * 100 if DayDeaMeSmA00S00`location' != .
        qui replace DayAPEMeSmA02S01`location'`update' = . if DayDeaMeSmA00S00`location' == . // before location was added to the estimates for the country
