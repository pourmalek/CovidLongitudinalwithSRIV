...
local list DELP IMPE LANL SRIV
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	replace DayDeaMeFoIMPE`update' = . if date < update_date`update' 
	...
	save "IMPE `update' Tanzania.dta", replace 
...
/* foreach update of local list {
	...
	* graph Tanzania IMPE daily deaths each update 
	...
	twoway ///
	(line DayDeaMeSmJOHNTZA date, sort lwidth(thick) lcolor(cyan)) /// 	
	(line DayDeaMeFoIMPE`update' date, sort lwidth(thin) lcolor(brown)) /// 
	if date >= td(01Jan2020) & date <= td(01Jan2023) ///
	, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
	xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Tanzania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Tanzania.dta"
...
save "SRIV Tanzania.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV Tanzania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tanzania SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Tanzania, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tanzania SRIV daily deaths update `update'.pdf", replace	
...
* graph Tanzania SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tanzania, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tanzania SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1446 "JOHN" 4 "DELP" 1 "IMPE" 24 "LANL" 7 "SRIV") ///
