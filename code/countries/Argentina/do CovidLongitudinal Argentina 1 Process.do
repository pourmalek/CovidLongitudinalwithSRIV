...
** graphs:
** "graph 2 Argentina `model' C19 daily deaths all updates.pdf"
** "graph 2 Argentina ALL MODELS C19 daily deaths all updates.pdf"
...
qui graph export "graph 2 Argentina JOHN C19 daily deaths reported.pdf", replace
...
qui graph export "graph 2 Argentina DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	*/
	...
	// qui graph export "graph Argentina 0 IHME daily deaths epoch `update'.pdf", replace
...
qui graph export "graph 2 Argentina IHME C19 daily deaths all updates.pdf", replace
...
qui graph export "graph 2 Argentina IMPE C19 daily deaths all updates.pdf", replace
...
qui graph export "graph 2 Argentina LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Argentina.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Argentina.dta"
...
save "SRIV Argentina.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV Argentina data dictionary.csv", replace 
...
* graph Argentina SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small)) ylabel(, labsize(small) angle(forty_five)) ///
ytitle(Daily deaths) title("C19 daily deaths, Argentina, SRIV, all updates", size(medium) color(black)) ///
legend(order(1 "JOHN" 2 "SRIV Forecast") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 2 Argentina SRIV C19 daily deaths all updates.pdf", replace
...
qui graph export "graph 2 Argentina UCLA C19 daily deaths all updates.pdf", replace
...
qui graph export "graph 2 Argentina YYGU C19 daily deaths all updates.pdf", replace
...
legend(order(1667 "JOHN" 1 "DELP" 167 "IHME" 4 "IMPE" 3 "LANL" 153 "SRIV" 678 "UCLA" 169 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 2 Argentina ALL MODELS C19 daily deaths all updates.pdf", replace
