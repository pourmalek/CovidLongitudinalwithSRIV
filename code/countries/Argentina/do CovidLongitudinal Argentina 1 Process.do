...
foreach update of local list {
	...
	* graph Argentina SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Argentina, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Argentina SRIV daily deaths update `update'.pdf", replace	
...
qui graph export "graph 02 Argentina SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1716 "JOHN" 1 "DELP" 34 "IHME" 4 "IMPE" 3 "LANL" 11 "SRIV" 614 "UCLA" 37 "YYGU") ///
