...
log using "log CovidLongitudinal Dominica 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Dominica 1 Process.do"
...
**** "do CovidLongitudinal Dominica 1 Process.do"
...
** output data files: "`model' Dominica.dta"
**                   "ALL MODLES Dominica.dta"

** output data dictionary files: "Dominica ALL MODELS update dates data dictionary.csv"
**                               "`model' Dominica data dictionary.csv"
**                               "ALL MODLES Dominica data dictionary.csv"

** graphs:
** "graph 02 Dominica `model' C19 daily deaths all updates.pdf"
** "graph 02 Dominica ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Dominica absent update_date_block"
	
	qui keep if loc_grand_name == "Dominica"
	...
	save "Dominica `model' update dates.dta", replace
...
use "Dominica SRIV update dates.dta", clear 

local list SRIV

foreach model of local list {
	
	append using "Dominica `model' update dates.dta"
...
save "Dominica ALL MODELS update dates.dta", replace
	
export excel using "Dominica ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Dominica ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Dominica"

keep date DayDeaMeSmJOHNDMA
...
save "JOHN Dominica.dta", replace 




* graph Dominica JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNDMA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Dominica, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Dominica JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Dominica.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Dominica.dta"
...
save "SRIV Dominica.dta", replace
...
merge m:1 date using "JOHN Dominica.dta"
...
preserve
    ...
    export delimited name varlab using "SRIV Dominica data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Dominica SRIV daily deaths each update 
	...
	subtitle("Dominica, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Dominica SRIV daily deaths update `update'.pdf", replace	
...
* graph Dominica SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Dominica, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Dominica SRIV C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Dominica.dta", force
...
save "ALL MODLES Dominica.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Dominica data dictionary.csv", replace 
...
* graph Dominica ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Dominica, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(861 "JOHN" 1 "SRIV") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Dominica ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Dominica 1 Process.smcl"
