...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Germany.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Germany.dta"
...
preserve
    ...
    export delimited name varlab using "SRIV Germany data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Germany SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Germany, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Germany SRIV daily deaths update `update'.pdf", replace	
...
* graph Germany SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Germany, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Germany SRIV C19 daily deaths all updates.pdf", replace
...
/* foreach update of local list {
	...
	* graph Germany UCLA daily deaths each update 
	...
	twoway ///
	(line DayDeaMeSmJOHNDEU date, sort lwidth(thick) lcolor(cyan)) /// 	
	(line DayDeaMeFoUCLA`update' date, sort lwidth(thin) lcolor(gold)) /// 
	if date >= td(01Jan2020) & date <= td(01Jan2023) ///
	, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
	xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
	ytitle(Daily deaths) title("C19 daily deaths, UCLA, update `update'", size(medium) color(black)) ///
...
legend(position(6) order(1726 "JOHN" 19 "DELP" 5 "IHME" 125 "IMPE" 33 "LANL" 45 "SRIV" 626 "UCLA" 1 "YYGU") ///
