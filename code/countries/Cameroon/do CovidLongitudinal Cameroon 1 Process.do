...
log using "log CovidLongitudinal Cameroon 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Cameroon 1 Process.do"
...
**** "do CovidLongitudinal Cameroon 1 Process.do"
...
** output data files: "`model' Cameroon.dta"
**                   "ALL MODLES Cameroon.dta"

** output data dictionary files: "Cameroon ALL MODELS update dates data dictionary.csv"
**                               "`model' Cameroon data dictionary.csv"
**                               "ALL MODLES Cameroon data dictionary.csv"

** graphs:
** "graph 02 Cameroon `model' C19 daily deaths all updates.pdf"
** "graph 02 Cameroon ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Cameroon absent update_date_block"
	
	qui keep if loc_grand_name == "Cameroon"
	...
	save "Cameroon `model' update dates.dta", replace
...
use "Cameroon DELP update dates.dta", clear 

local list DELP IHME IMPE LANL SRIV

foreach model of local list {
	
	append using "Cameroon `model' update dates.dta"
...
save "Cameroon ALL MODELS update dates.dta", replace
	
export excel using "Cameroon ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Cameroon ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Cameroon"

keep date DayDeaMeSmJOHNCMR
...
save "JOHN Cameroon.dta", replace 




* graph Cameroon JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCMR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Cameroon, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cameroon JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Cameroon.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Cameroon.dta"
...
save "DELP Cameroon.dta", replace
...
merge m:m date using "JOHN Cameroon.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Cameroon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cameroon DELP daily deaths each update 
	...
	subtitle("Cameroon, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cameroon DELP daily deaths update `update'.pdf", replace	
...
* graph Cameroon DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cameroon, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cameroon DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Cameroon IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Cameroon, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Cameroon IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Cameroon.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Cameroon.dta"
...
save "IHME Cameroon.dta", replace
...
merge m:1 date using "JOHN Cameroon.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Cameroon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cameroon IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cameroon, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cameroon IHME daily deaths update `update'.pdf", replace	
...
* graph Cameroon IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cameroon, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cameroon IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Cameroon.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Cameroon.dta"
...
save "IMPE Cameroon.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Cameroon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cameroon IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cameroon, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cameroon IMPE daily deaths update `update'.pdf", replace	
...
* graph Cameroon IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cameroon, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cameroon IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Cameroon.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Cameroon.dta"
...
save "LANL Cameroon.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Cameroon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cameroon LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cameroon, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cameroon LANL daily deaths update `update'.pdf", replace	
...
* graph Cameroon LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cameroon, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cameroon LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Cameroon.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Cameroon.dta"
...
save "SRIV Cameroon.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV Cameroon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cameroon SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cameroon, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cameroon SRIV daily deaths update `update'.pdf", replace	
...
* graph Cameroon SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cameroon, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cameroon SRIV C19 daily deaths all updates.pdf", replace
...
use "DELP Cameroon.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Cameroon.dta", force
...
save "ALL MODLES Cameroon.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Cameroon data dictionary.csv", replace 
...
* graph Cameroon ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cameroon, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(1530 "JOHN" 1 "DELP" 127 "IHME" 4 "IMPE" 3 "LANL" 11 "SRIV") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cameroon ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Cameroon 1 Process.smcl"
