...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Central African Republic.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Central African Republic.dta"
...
save "SRIV Central African Republic.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV Central African Republic data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Central African Republic SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Central African Republic, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Central African Republic SRIV daily deaths update `update'.pdf", replace	
...
* graph Central African Republic SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Central African Republic, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Central African Republic SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1485 "JOHN" 16 "DELP" 82 "IHME" 25 "IMPE" 75 "LANL" 1 "SRIV") ///
