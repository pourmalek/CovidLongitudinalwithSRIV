...
foreach update of local list {
	...
	* graph United States of America SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("United States of America, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 United States of America SRIV daily deaths update `update'.pdf", replace	
...
qui graph export "graph 02 United States of America SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1743 "JOHN" 29 "DELP" 1 "IHME" 135 "IMPE" 43 "LANL" 55 "SRIV" 635 "UCLA" 8 "YYGU") ///
