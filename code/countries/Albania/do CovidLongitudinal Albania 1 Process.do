...
** graphs:
** "graph 2 Albania `model' C19 daily deaths all updates.pdf"
** "graph 2 Albania ALL MODELS C19 daily deaths all updates.pdf"
...
qui graph export "graph 2 Albania JOHN C19 daily deaths reported.pdf", replace
...
qui graph export "graph 2 Albania DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	*/
	...
	// qui graph export "graph Albania 0 IHME daily deaths epoch `update'.pdf", replace
...
qui graph export "graph 2 Albania IHME C19 daily deaths all updates.pdf", replace
...
qui graph export "graph 2 Albania IMPE C19 daily deaths all updates.pdf", replace
...
qui graph export "graph 2 Albania LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Albania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Albania.dta"
...
preserve
    ...
    export delimited name varlab using "SRIV Albania data dictionary.csv", replace 
...
* graph Albania SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small)) ylabel(, labsize(small) angle(forty_five)) ///
ytitle(Daily deaths) title("C19 daily deaths, Albania, SRIV, all updates", size(medium) color(black)) ///
legend(order(1 "JOHN" 2 "SRIV Forecast") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 2 Albania SRIV C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Albania.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Albania.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Albania.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Albania data dictionary.csv", replace 
...
* graph Albania UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small)) ylabel(, labsize(small) angle(forty_five)) ///
ytitle(Daily deaths) title("C19 daily deaths, Albania, UCLA, all updates", size(medium) color(black)) ///
legend(order(1 "JOHN" 2 "UCLA Forecast") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 2 Albania UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Albania.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Albania.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Albania.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Albania data dictionary.csv", replace 
	
restore



* graph Albania YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small)) ylabel(, labsize(small) angle(forty_five)) ///
ytitle(Daily deaths) title("C19 daily deaths, Albania, YYGU, all updates", size(medium) color(black)) ///
legend(order(1 "JOHN" 2 "YYGU Forecast") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 2 Albania YYGU C19 daily deaths all updates.pdf", replace
...
legend(order(1480 "JOHN" 1 "DELP" 127 "IHME" 4 "IMPE" 3 "LANL" 11 "SRIV") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 2 Albania ALL MODELS C19 daily deaths all updates.pdf", replace
