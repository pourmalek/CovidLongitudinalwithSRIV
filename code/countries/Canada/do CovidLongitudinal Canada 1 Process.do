...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Canada.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Canada.dta"
...
preserve
    ...
    export delimited name varlab using "SRIV Canada data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Canada SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Canada, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Canada SRIV daily deaths update `update'.pdf", replace	
...
* graph Canada SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Canada, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Canada SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1718 "JOHN" 11 "DELP" 17 "IHME" 115 "IMPE" 23 "LANL" 35 "SRIV" 616 "UCLA" 1 "YYGU") ///
