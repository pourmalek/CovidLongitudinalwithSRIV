...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Ethiopia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Ethiopia.dta"
...
save "SRIV Ethiopia.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV Ethiopia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ethiopia SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ethiopia, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ethiopia SRIV daily deaths update `update'.pdf", replace	
...
* graph Ethiopia SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ethiopia, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ethiopia SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1512 "JOHN" 4 "DELP" 161 "IHME" 1 "IMPE" 86 "LANL" 7 "SRIV") ///
