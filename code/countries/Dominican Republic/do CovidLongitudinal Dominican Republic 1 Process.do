...
local list DELP IHME IMPE LANL SRIV YYGU
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Dominican Republic.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Dominican Republic.dta"
...
save "SRIV Dominican Republic.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV Dominican Republic data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Dominican Republic SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Dominican Republic, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Dominican Republic SRIV daily deaths update `update'.pdf", replace	
...
* graph Dominican Republic SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Dominican Republic, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Dominican Republic SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1686 "JOHN" 1 "DELP" 34 "IHME" 4 "IMPE" 3 "LANL" 11 "SRIV" 37 "YYGU") ///
