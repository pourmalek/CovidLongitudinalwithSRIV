...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' South Sudan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' South Sudan.dta"
...
save "SRIV South Sudan.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV South Sudan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph South Sudan SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("South Sudan, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 South Sudan SRIV daily deaths update `update'.pdf", replace	
...
* graph South Sudan SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, South Sudan, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 South Sudan SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1495 "JOHN" 18 "DELP" 92 "IHME" 14 "IMPE" 69 "LANL" 1 "SRIV") ///
