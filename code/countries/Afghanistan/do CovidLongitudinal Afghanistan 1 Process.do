...
* Objective: Process the reported deaths by JOHN and 
	...
	* for each country ---->> Afghanistan / AFG <<----                                                                 
***************************************************************************
...
** graphs:
** "graph 2 Afghanistan `model' C19 daily deaths all updates.pdf"
** "graph 2 Afghanistan ALL MODELS C19 daily deaths all updates.pdf"
...
qui graph export "graph 2 Afghanistan JOHN C19 daily deaths reported.pdf", replace
...
qui graph export "graph 2 Afghanistan DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	*/
	...
	// qui graph export "graph Afghanistan 0 IHME daily deaths epoch `update'.pdf", replace
...
qui graph export "graph 2 Afghanistan IHME C19 daily deaths all updates.pdf", replace
...
qui graph export "graph 2 Afghanistan IMPE C19 daily deaths all updates.pdf", replace
...
qui graph export "graph 2 Afghanistan LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Afghanistan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Afghanistan.dta"
...
save "SRIV Afghanistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV Afghanistan data dictionary.csv", replace 
...
* graph Afghanistan SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small)) ylabel(, labsize(small) angle(forty_five)) ///
ytitle(Daily deaths) title("C19 daily deaths, Afghanistan, SRIV, all updates", size(medium) color(black)) ///
legend(order(1 "JOHN" 2 "SRIV Forecast") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 2 Afghanistan SRIV C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Afghanistan.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Afghanistan.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Afghanistan.dta"
...
save "UCLA Afghanistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "UCLA Afghanistan data dictionary.csv", replace 
...
* graph Afghanistan UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small)) ylabel(, labsize(small) angle(forty_five)) ///
ytitle(Daily deaths) title("C19 daily deaths, Afghanistan, UCLA, all updates", size(medium) color(black)) ///
legend(order(1 "JOHN" 2 "UCLA Forecast") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 2 Afghanistan UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Afghanistan.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Afghanistan.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Afghanistan.dta"
...
save "YYGU Afghanistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Afghanistan data dictionary.csv", replace 
	
restore



* graph Afghanistan YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small)) ylabel(, labsize(small) angle(forty_five)) ///
ytitle(Daily deaths) title("C19 daily deaths, Afghanistan, YYGU, all updates", size(medium) color(black)) ///
legend(order(1 "JOHN" 2 "YYGU Forecast") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 2 Afghanistan YYGU C19 daily deaths all updates.pdf", replace
...
legend(order(1478 "JOHN" 1 "DELP" 126 "IHME" 4 "IMPE" 3 "LANL" 11 "SRIV") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 2 Afghanistan ALL MODELS C19 daily deaths all updates.pdf", replace
