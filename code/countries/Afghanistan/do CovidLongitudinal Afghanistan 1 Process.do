...
/* foreach update of local list {
	...
	* graph Afghanistan SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Afghanistan, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Afghanistan SRIV daily deaths update `update'.pdf", replace	
...
qui graph export "graph 02 Afghanistan SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1528 "JOHN" 1 "DELP" 126 "IHME" 4 "IMPE" 3 "LANL" 11 "SRIV") ///
