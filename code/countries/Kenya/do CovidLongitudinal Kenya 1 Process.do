...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Kenya.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Kenya.dta"
...
preserve
    ...
    export delimited name varlab using "SRIV Kenya data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kenya SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kenya, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kenya SRIV daily deaths update `update'.pdf", replace	
...
* graph Kenya SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kenya, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kenya SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1524 "JOHN" 1 "DELP" 124 "IHME" 3 "IMPE" 17 "LANL" 9 "SRIV") ///
