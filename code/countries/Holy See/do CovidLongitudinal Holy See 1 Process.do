...
log using "log CovidLongitudinal Holy See 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Holy See 1 Process.do"
...
**** "do CovidLongitudinal Holy See 1 Process.do"
...
** output data files: "`model' Holy See.dta"
**                   "ALL MODLES Holy See.dta"

** output data dictionary files: "Holy See ALL MODELS update dates data dictionary.csv"
**                               "`model' Holy See data dictionary.csv"
**                               "ALL MODLES Holy See data dictionary.csv"

** graphs:
** "graph 02 Holy See `model' C19 daily deaths all updates.pdf"
** "graph 02 Holy See ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Holy See absent update_date_block"
	
	qui keep if loc_grand_name == "Holy See"
	...
	save "Holy See `model' update dates.dta", replace
...
use "Holy See SRIV update dates.dta", clear 

local list SRIV

foreach model of local list {
	
	append using "Holy See `model' update dates.dta"
...
save "Holy See ALL MODELS update dates.dta", replace
	
export excel using "Holy See ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Holy See ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Holy See"

keep date DayDeaMeSmJOHNVAT
...
save "JOHN Holy See.dta", replace 




* graph Holy See JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNVAT date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Holy See, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Holy See JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Holy See.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Holy See.dta"
...
save "SRIV Holy See.dta", replace
...
merge m:1 date using "JOHN Holy See.dta"
...
preserve
    ...
    export delimited name varlab using "SRIV Holy See data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Holy See SRIV daily deaths each update 
	...
	subtitle("Holy See, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Holy See SRIV daily deaths update `update'.pdf", replace	
...
* graph Holy See SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Holy See, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Holy See SRIV C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Holy See.dta", force
...
save "ALL MODLES Holy See.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Holy See data dictionary.csv", replace 
...
* graph Holy See ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Holy See, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(861 "JOHN" 1 "SRIV") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Holy See ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Holy See 1 Process.smcl"
