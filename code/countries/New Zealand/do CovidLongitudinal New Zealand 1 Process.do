...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' New Zealand.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' New Zealand.dta"
...
save "SRIV New Zealand.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV New Zealand data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph New Zealand SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("New Zealand, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 New Zealand SRIV daily deaths update `update'.pdf", replace	
...
* graph New Zealand SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, New Zealand, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 New Zealand SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1488 "JOHN" 1 "DELP" 87 "IHME" 46 "IMPE" 8 "LANL" 4 "SRIV") ///
