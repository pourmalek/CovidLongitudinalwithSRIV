...
log using "log CovidLongitudinal Saint Kitts and Nevis 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Saint Kitts and Nevis 1 Process.do"
...
**** "do CovidLongitudinal Saint Kitts and Nevis 1 Process.do"
...
** output data files: "`model' Saint Kitts and Nevis.dta"
**                   "ALL MODLES Saint Kitts and Nevis.dta"

** output data dictionary files: "Saint Kitts and Nevis ALL MODELS update dates data dictionary.csv"
**                               "`model' Saint Kitts and Nevis data dictionary.csv"
**                               "ALL MODLES Saint Kitts and Nevis data dictionary.csv"

** graphs:
** "graph 02 Saint Kitts and Nevis `model' C19 daily deaths all updates.pdf"
** "graph 02 Saint Kitts and Nevis ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Saint Kitts and Nevis absent update_date_block"
	
	qui keep if loc_grand_name == "Saint Kitts and Nevis"
	...
	save "Saint Kitts and Nevis `model' update dates.dta", replace
...
use "Saint Kitts and Nevis SRIV update dates.dta", clear 

local list SRIV

foreach model of local list {
	
	append using "Saint Kitts and Nevis `model' update dates.dta"
...
save "Saint Kitts and Nevis ALL MODELS update dates.dta", replace
	
export excel using "Saint Kitts and Nevis ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Saint Kitts and Nevis ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Saint Kitts and Nevis"

keep date DayDeaMeSmJOHNKNA
...
save "JOHN Saint Kitts and Nevis.dta", replace 




* graph Saint Kitts and Nevis JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNKNA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Saint Kitts and Nevis, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Saint Kitts and Nevis JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Saint Kitts and Nevis.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Saint Kitts and Nevis.dta"
...
save "SRIV Saint Kitts and Nevis.dta", replace
...
merge m:1 date using "JOHN Saint Kitts and Nevis.dta"
...
preserve
    ...
    export delimited name varlab using "SRIV Saint Kitts and Nevis data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Saint Kitts and Nevis SRIV daily deaths each update 
	...
	subtitle("Saint Kitts and Nevis, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Saint Kitts and Nevis SRIV daily deaths update `update'.pdf", replace	
...
* graph Saint Kitts and Nevis SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Saint Kitts and Nevis, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Saint Kitts and Nevis SRIV C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Saint Kitts and Nevis.dta", force
...
save "ALL MODLES Saint Kitts and Nevis.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Saint Kitts and Nevis data dictionary.csv", replace 
...
* graph Saint Kitts and Nevis ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Saint Kitts and Nevis, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(861 "JOHN" 1 "SRIV") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Saint Kitts and Nevis ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Saint Kitts and Nevis 1 Process.smcl"
