...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Papua New Guinea.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Papua New Guinea.dta"
...
save "SRIV Papua New Guinea.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV Papua New Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Papua New Guinea SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Papua New Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Papua New Guinea SRIV daily deaths update `update'.pdf", replace	
...
* graph Papua New Guinea SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Papua New Guinea, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Papua New Guinea SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1301 "JOHN" 114 "DELP" 55 "IHME" 93 "IMPE" 378 "LANL" 1 "SRIV") ///
