...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Saudi Arabia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Saudi Arabia.dta"
...
save "SRIV Saudi Arabia.dta", replace
...
preserve
    ...
    export delimited name varlab using "SRIV Saudi Arabia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Saudi Arabia SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Saudi Arabia, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Saudi Arabia SRIV daily deaths update `update'.pdf", replace	
...
* graph Saudi Arabia SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Saudi Arabia, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Saudi Arabia SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1664 "JOHN" 1 "DELP" 133 "IHME" 68 "IMPE" 3 "LANL" 6 "SRIV" 562 "UCLA" 20 "YYGU") ///
