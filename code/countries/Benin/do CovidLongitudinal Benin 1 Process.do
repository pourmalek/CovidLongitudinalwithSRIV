...
(line DayDeaMeFoDELP20220915 date, sort lwidth(thin) lcolor(red)) ///(line DayDeaMeFoDELP20200517 date, sort lwidth(thin) lcolor(red)) ///
(line DayDeaMeFoDELP20200524 date, sort lwidth(thin) lcolor(red)) ///
(line DayDeaMeFoDELP20200531 date, sort lwidth(thin) lcolor(red)) ///
(line DayDeaMeFoDELP20200607 date, sort lwidth(thin) lcolor(red)) ///
(line DayDeaMeFoDELP20200614 date, sort lwidth(thin) lcolor(red)) ///
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' Benin.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' Benin.dta"
...
preserve
    ...
    export delimited name varlab using "SRIV Benin data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Benin SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Benin, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Benin SRIV daily deaths update `update'.pdf", replace	
...
* graph Benin SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Benin, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Benin SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1507 "JOHN" 35 "DELP" 105 "IHME" 1 "IMPE" 116 "LANL" 6 "SRIV") ///
