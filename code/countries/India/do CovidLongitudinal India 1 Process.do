...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "SRIV `update' India.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "SRIV `update' India.dta"
...
preserve
    ...
    export delimited name varlab using "SRIV India data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph India SRIV daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("India, forecast only") legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 India SRIV daily deaths update `update'.pdf", replace	
...
* graph India SRIV daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, India, SRIV, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "SRIV forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 India SRIV C19 daily deaths all updates.pdf", replace
...
legend(position(6) order(1732 "JOHN" 11 "DELP" 34 "IHME" 398 "IMPE" 22 "LANL" 36 "SRIV" 630 "UCLA" 1 "YYGU") ///
