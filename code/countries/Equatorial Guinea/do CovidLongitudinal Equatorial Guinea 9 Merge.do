...
use "DELP Equatorial Guinea error.dta", clear 
	...
	merge m:m date using "SRIV Equatorial Guinea error.dta"	
...
legend(position(6) order(19 "DELP" 108 "IHME" 1 "IMPE" 93 "LANL" 6 "SRIV") ///
...
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
subtitle("Equatorial Guinea, all Models, all updates, forecast only", size(small)) ///
legend(position(6) order(1 "DELP" 126 "IHME" 4 "IMPE" 3 "LANL" 11 "SRIV") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand)
